  ...
  local function updateData()
    ...
    local newPos
      ...
      local oldStart, oldHeight = item.startsAt, item.height
        ...
        if not newPos then ui.setCursorY(curPos) end
        newPos = render(item.wrapped, posX, curPos, notify)
        local newHeight = (newPos or ui.getCursorY()) - curPos
    ...
    if maxCurPos == -1 then
      ...
      maxCurPos = (maxCurPos * 9 + curPos) / 10
    end
