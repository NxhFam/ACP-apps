...
Given an FFI struct, returns bytes with its content. Resulting string may contain zeroes.
  ...
  - `binary|nil` If data is `nil`, returns `nil`.
...
Sets a callback which will be called when a file changes.
  ...
  1. `filename`: `string` Full path to a file to monitor.
...
Changes location of script storage. Available only for scripts with I/O access.
  ...
  1. `relativePath`: `string` Path relative to directory with Lua configs*/.
...
To clear cached image and free memory, pass returned value to `ui.unloadImage()`.
  ...
  - `string`
## Function ui.drawImage(imageSource, p1, p2, color, uv1, uv2, keepAspectRatio)
...
Draw loading animation spinner. Actual shape might change in the future versions.
  ...
  1. `p1`: `vec2` Position of upper left corner relative to current working area (scriptable texture or IMGUI window).

  2. `p2`: `vec2` Position of bottom right corner relative to current working area (scriptable texture or IMGUI window).
...
Draws text in a given rect with clipping and optional alignment without advancing cursor or anything like that. Faster option.
  ...
  5. `alignment`: `vec2?` Default value: `vec2(0, 0)`.
...
Draws TTF text in a given rect with clipping and optional alignment without advancing cursor or anything like that. Faster option.
  ...
  5. `horizontalAligment`: `ui.Alignment?` \`ac.Alignment.Start\` for left, \`ac.Alignment.Center\` for middle, \`ac.Alignment.End\` for right. Default value: `alignment::center`.

  6. `verticalAlignment`: `ui.Alignment?` \`ac.Alignment.Start\` for top, \`ac.Alignment.Center\` for middle, \`ac.Alignment.End\` for bottom. Default value: `alignment::center`.
...
Add notification counter (small red circle with a bit of a glow and an optional digit in it).
  ...
  2. `pos`: `vec2|nil` If not set, notification counter will be added to the previous element. Default value: `nil`.
...
Base IMGUI popup model for something more custom.
  ...
  1. `id`: `string`
## Function ui.beginPopup(id, flags)

  Parameters:

  1. `id`: `string`

  2. `flags`: `ui.WindowFlags?` Default value: 0.
...
Was the last item selection (`ui.selectable()`, `ui.treeNode()`, etc.) toggled
  ...
  - `boolean`
## Function ui.image(imageSource, size, color, borderColor, uv1, uv2, keepAspectRatio)
...
`ui.getCursor()` for `p1`, `ui.getCursor() + size` for `p2` and `ui.dummy()` for interaction.
  ...
  7. `keepAspectRatio`: `boolean?` Set to \`true\` to stretch image to fit given size, making sure it would not get distorted. Default value: `false`.
## Function ui.imageButton(imageSource, size, bgColor, color, uv1, uv2, framePadding, keepAspectRatio)
...
like ice.
  ...
  2. `wheels`: `ac.Wheel` Wheels to affect.

  3. `decrease`: `number` 0 for tyres to act as usual, 1 for tyres to lose all grip.
...
## Function physics.resetTrackPhysicsObject(name)
...
Blows up some tyres of a certain car. Invalidates current lap.
  ...
  2. `wheels`: `ac.Wheel?` Wheels to affect. Default value: 15.
...
## Function physics.setTyresTemperature(carIndex, wheels, temperature)
Sets tyres temperature and invalidates current lap. To get optimal tyre temperature, use `ac.getCar(N).wheels[0].tyreOptimumTemperature`.
  ...
  3. `temperature`: `number` New tyres temperature in °C.
...
Sets driven distance used for wear computation.
  ...
  3. `virtualKm`: `number` Driven distance.
...
## Function physics.disableCarCollisions(carIndex, disable)
Enables and disables car collisions (wouldn’t affect disabled collisions in pits, more like an extra option to
 further disable collisions). Invalidates lap.
...
Stops car from sleeping, invalidates current lap.
  ...
  1. `carIndex`: `integer` 0-based car index.
## Function physics.addForce(carIndex, position, posLocal, force, forceLocal)
...
Changes AI level. Use `ac.getCar(carIndex).aiLevel` to get current value.
  ...
  2. `level`: `number` AI level from 0 to 1.
...
Changes AI aggression. Use `ac.getCar(carIndex).aiAggression` to get current value.
  ...
  2. `aggression`: `number` AI aggression from 0 to 1.
...
## Function ui.onDriverTooltip(callback)
Sets a callback which will be called when drawing driver tooltip card. Within, `ui.` functions can be used to add additional information.
  ...
  1. `callback`: `fun(carIndex: integer)` Callback which will be called each time tooltip is drawn, getting 0-based car index.
...
## Function ac.isPythonAppActive(appName)
  ...
  - `boolean`
## Function ac.reloadPythonApp(appName)
...
Gets current VAO mode, meant to be used for debugging purposes only.
  ...
  - `ac.VAODebugMode`
## Function ac.setCameraCPLSettings(intensity, rotation, offset, photoelasticityBoost)
Override camera CPL settings.
  ...
  4. `photoelasticityBoost`: `number?` Boost for glass photoelasticity effect, from 0 to 3. Default value: 1.
...
Override scene tweaks.
  ...
  - `ac.SceneTweaks`
## Function ac.setAudioVolume(audioChannelKey, value)
...
## Function ac.setTrackCoordinatesDeg(coords)
Sets track world coordinates in degrees.

  Parameters:

  1. `coords`: `vec2` X for lattitude, Y for longitude.
...
Replace driver model with a custom one asyncronously. For method to work current and new driver models should have common skeleton.
  ...
  2. `data`: `binary|nil` Use \`nil\` to restore original model. Default value: `nil`.
...
command (return it if your script recognizes command to stop further processing).
  ...
  1. `callback`: `fun(command: string): boolean` Callback which will be called each time user enters new command.
...
Use this function to fade out volume for one reason or another.
  ...
  1. `callback`: `fun(): number` Callback which will be called each time user enters new command.
...
Runs asyncronously.
  ...
  2. `format`: `ac.ScreenshotFormat?` Default value: 0.

  3. `callback`: `nil|fun(err: string)?` Default value: 0.
...
Refreshes list of setups. Any listeners set with `ac.onSetupsListRefresh()` will be called as well.
  ...
  - `boolean`
## Function ac.saveCurrentSetup(setupName)
Saves current setup into a file. If successful, refreshes setups list and calls any listeners set with `ac.onSetupsListRefresh()` as well.
  ...
  1. `setupName`: `string` Absolute path, or path relative to “Documents\Assetto Corsa\setups\<car ID>”.

  Returns:

  - `boolean`
## Function ac.loadSetup(setupName)
Loads car setup from a file. Works only from setup menu, and only if setup is not fixed.
  ...
  - `boolean`
## Function ac.resetSetupToDefault(setupName)
...
Move mouse to a certain position.
  ...
  1. `posPixels`: `vec2` New mouse cursor position in pixels relative to AC window.
...
Can be used for simulating mouse wheel.
  ...
  1. `scroll`: `number`
## Function ac.startBackgroundWorker(name, data, callback)
...
manually (just add a new line with it in “.vscode/settings.json”.
  ...
  1. `name`: `string` File name (without extension) relative to script directory.
  ...
  3. `callback`: `fun(err: string, data: string|number|boolean)`
...
Clones table using a fast LuaJIT call.
  ...
  2. `deep`: `boolean|`true`|`false`` Set to `true` for deep cloning.
...
## Function render.PositioningHelper()
...
Creates a new layer with user icons.
  ...
  1. `priority`: `number` Layer with higher priority will be used.
...
- `ac.SceneReference:setTransformationFrom(rigidBody, localTransform)`
...
There is also a method `physics.RigidBody:setTransformationFrom()` doing the opposite (and requiring an inverse of this matrix).
  ...
  Parameters:
    ...
    1. `rigidBody`: `physics.RigidBody` Physics entity to sync transformation with.

    2. `localTransform`: `mat4x4?` Optional transformation of scene reference nodes relative to the physics entity.
...
- `physics.RigidBody:setDamping(linear, angular)`
...
Represents a physics rigid body. Requires double precision physics engine to work.
  ...
  1. `collider`: `string|physics.ColliderType[]` Collider KN5 filename, or a table listing geometric colliders (see `physics.Collider`).

  2. `mass`: `number` Mass in kg, can be changed later.

  3. `cog`: `vec3?` Center of gravity in collider model, can’t be changed later.

  4. `semiDynamic`: `boolean?` Semi-dynamic from the start. Default value: `false`.
...
For internal use.
  ...
  3. `modifiers`: `ac.ControlButtonModifiers?`

  4. `repeatPeriod`: `number?`
...
# Module wfx_common/ac_weatherconditions.lua
