...
## Function ac.setManualCloudsInvalidation(value)
  ...
  1. `value`: `boolean|`true`|`false``
## Function ac.enableRenderCallback(sky, clouds, track, cloudShadows)
...
main pass ID.
  ...
  4. `cloudShadows`: `render.PassID?` Default value: 0.
...
- `ScriptData.renderSky(passID, frameIndex, uniqueKey)`

  Called right after the sky (and sky covers) were rendered. Use `ac.enableRenderCallback()` to activate.

  Parameters:

    1. `passID`: `render.PassID`

    2. `frameIndex`: `integer`

    3. `uniqueKey`: `integer`

- `ScriptData.renderClouds(passID, frameIndex, uniqueKey)`
...
- `ScriptData.renderCloudShadows(passID, frameIndex, uniqueKey)`
