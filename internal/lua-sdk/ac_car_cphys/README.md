...
## Function ac.addTorque(torque, torqueLocal)
Adds force to a car body, invalidates current lap.

  Parameters:

  1. `torque`: `vec3` Force vector in N.

  2. `torqueLocal`: `boolean|`true`|`false`` If \`true\`, force is treated like vector relative to car coordinates, otherwise as world vector.
...
## Function ac.shiftPatchTemperatures(tyre, targetTemperature, transfer)
