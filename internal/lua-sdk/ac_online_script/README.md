...
user selects a feature from the list.
  ...
  3. `availableCallback`: `nil|fun(): boolean` Optional callback to check if feature is available.

  4. `uiCallback`: `nil|fun()` Optional callback to build options UI.

  5. `closeCallback`: `nil|fun(okClicked: boolean)` Optional callback to call to apply changes.

  6. `flags`: `ui.OnlineExtraFlags` Flags for specialized behaviour.

  7. `toolFlags`: `ui.WindowFlags?` Tool window flags (only with tool mode). Default value: `ImGuiWindowFlags_None`.

  8. `toolSize`: `vec2?` Default tool size (only with tool mode). Default value: `vec2(320, 400)`.
...
- `ac.TrackCondition:get()`

  Returns:

    - `number`

- `ac.TrackCondition:getColor()`
